...
class Fotografi extends CI_Controller
{
    ...
    public function index(){
        ...
        $lombacheck = $this->M_lomba->daftarcheckf($id_user);
        $verifcheck = $this->M_lomba->verifcheckf($id_user);
        $verifpk = $this->M_lomba->verifpkf($id_user);
        $data['user'] = $this->db->get_where('user', ['email' => $this->session->userdata('email')])->row_array();
        $this->load->view('peserta/templates/header',$data);
        if($lombacheck){
            $this->load->view('peserta/fotografi/waitverif');
        }elseif($verifcheck && $waktu_sekarang > $waktu_acara){
            $this->load->view('peserta/fotografi/pengumpulankarya'); 
        }elseif($verifcheck){
            $this->load->view('peserta/fotografi/tm'); 
        }elseif($verifpk){
            $this->load->view('peserta/fotografi/pengumuman');
        }else{
            $this->load->view('peserta/fotografi/pendaftaran'); 
        }
