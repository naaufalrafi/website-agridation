...
class Cvideo extends CI_Controller
{
    ...
    public function index(){
        ...
        $lombacheck = $this->M_lomba->daftarcheckc($id_user);
        $verifcheck = $this->M_lomba->verifcheckc($id_user);
        $verifpk = $this->M_lomba->verifpkc($id_user);
        $data['user'] = $this->db->get_where('user', ['email' => $this->session->userdata('email')])->row_array();
        $this->load->view('peserta/templates/header',$data);
        if($lombacheck){
            $this->load->view('peserta/cvideo/waitverif');
        }elseif($verifcheck && $waktu_sekarang > $waktu_acara){
            $this->load->view('peserta/cvideo/pengumpulankarya'); 
        }elseif($verifcheck){
            $this->load->view('peserta/cvideo/tm'); 
        }elseif($verifpk){
            $this->load->view('peserta/cvideo/pengumuman');
        }else{
            $this->load->view('peserta/cvideo/pendaftaran'); 
        }
